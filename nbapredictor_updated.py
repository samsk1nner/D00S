...
class NBAPredictor:
    def fetch_team_stats(self, season='2024-25'):
        ...
        try:
            ...
            if 'TEAM_ABBREVIATION' in stats.columns and 'TEAM_ABBREVIATION' in elo_ratings.columns:
                ...
                print("Warning: Unable to merge ELO ratings due to missing team identifiers.")
    ...
    def calculate_four_factors(self, stats_df):
        ...
        try:
            ...
            stats_df['OREB%'] = stats_df['OREB'] / (stats_df['OREB'] + stats_df['OPP_DREB'])
    ...
    def fetch_game_logs(self, season='2024-25'):
        ...
        try:
            ...
            game_log.sort_values(['TEAM_ID', 'GAME_DATE'], inplace=True)
    ...
    def prepare_game_features(self, game_logs, team_stats):
        try:
            if 'TEAM_ID' not in game_logs.columns or 'TEAM_ABBREVIATION' not in team_stats.columns:
            ...
            features = pd.merge(
                features,
                team_stats[['TEAM_ID', 'TEAM_ABBREVIATION', 'eFG%', 'TOV%', 'OREB%', 'FTRate', 'FourFactorsScore', 'ELO']],
            ...
            for feature in rolling_features:
                features[f'{feature}_ROLLING_10'] = (
                    features.groupby('TEAM_ID')[feature]
...
def compile_nba_team_data(self, season='2024-25'):
    ...
    # Merge game logs with team statistics on TEAM_ID and TEAM_ABBREVIATION
    ...
    if 'team_id' in game_logs.columns and 'team_id' in team_stats.columns:
        combined_data = pd.merge(game_logs, team_stats, on=['team_id', 'TEAM_ABBREVIATION'], how='left', suffixes=('', '_team'))
    elif 'TEAM_ABBREVIATION' in game_logs.columns and 'TEAM_ABBREVIATION' in team_stats.columns:
        combined_data = pd.merge(game_logs, team_stats, on='TEAM_ABBREVIATION', how='left', suffixes=('', '_team'))
    else:
        print("Error: Missing necessary identifiers ('team_id' or 'TEAM_ABBREVIATION') for merging.")
        return pd.DataFrame()
    ...
    def predict_upcoming_games(self, recent_games, team_stats):
        ...
        for factor in ['eFG%', 'TOV%', 'OREB%', 'FTRate', 'FourFactorsScore']:
            upcoming_games[f'{factor}_ROLLING_10'] = (
                upcoming_games.groupby('team_id')[factor]
                .rolling(10, min_periods=1)
                .mean()
                .reset_index(0, drop=True)
            )
...
class NBAPredictor:
    ...
    def fetch_todays_games(self):
        ...
        try:
            ...
            return games_today[['TEAM_ID', 'TEAM_ABBREVIATION', 'MATCHUP']]
        except Exception as e:
